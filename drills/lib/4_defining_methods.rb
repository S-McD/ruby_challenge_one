...
# returns the total
def add_number_strings(num, num2)
...
# multiplies one by the other
# returns the result
def multiply_number_strings(num, num2)
    return (num.to_i) * (num2.to_i)
end
...
def both_odd?(num, num2)
    num.odd? && num2.odd? ? true : false
end
...
# otherwise, returns false
def both_even?(num, num2)
    num.even? && num2.even? ? true : false
end
...
def truncate(string)
    string.length > 10 ? string[0..9] + '...' : string
end